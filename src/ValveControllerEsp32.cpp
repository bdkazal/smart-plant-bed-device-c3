...
static const int VALVE_PIN = 26;
static const int VALVE_ON_LEVEL = HIGH;
static const int VALVE_OFF_LEVEL = LOW;
...
void writeValveLevel(int level)
{
  gpio_set_level((gpio_num_t)VALVE_PIN, level == HIGH ? 1 : 0);
}
...
{
  writeValveLevel(VALVE_OFF_LEVEL);
...
{
  writeValveLevel(VALVE_ON_LEVEL);
...
void beginValveOutput()
{
  ...
  Serial.println();
  Serial.println("Valve output initialized for ESP32 DevKit.");
...
void updateWateringState()
{
  ...
  if (millis() - wateringStartedAt < wateringDurationMs)
  {
    return;
  }
